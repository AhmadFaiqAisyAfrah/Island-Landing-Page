...
export default function PngToJpg() {
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `converted-${index + 1}.jpg`;
    link.click();
  };
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 max-w-2xl mx-auto py-12 px-4">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-3 font-serif">PNG to JPG</h1>
          <p className="text-gray-600">Convert PNG images to JPG format instantly</p>
        </div>

        <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => inputRef.current?.click()}
          className={`bg-white rounded-2xl border-2 border-dashed p-8 mb-6 cursor-pointer transition-all ${isDragging ? "border-pink-500 bg-pink-50" : "border-gray-300 hover:border-pink-400"}`}>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-gray-400 text-sm">PNG, JPG, WebP supported</span>
          </div>
        </div>
        ...
        {files.length > 0 && !convertedUrls.length && (
          <button onClick={convert} disabled={loading} className="w-full py-4 bg-pink-500 text-white font-semibold rounded-xl hover:bg-pink-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" />Converting...</> : <><CheckCircle className="w-5 h-5" />Convert to JPG</>}
          </button>
        )}
        ...
        {convertedUrls.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => convertedUrls.forEach((url, i) => downloadImage(url, i))} className="bg-pink-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-pink-600">
