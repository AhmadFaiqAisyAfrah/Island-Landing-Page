...
export default function JpgToPng() {
  ...
  const convertToPng = async () => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const pngUrl = await convertToPngFormat(dataUrl);
        results.push(pngUrl);
      }
  ...
  const convertToPngFormat = (dataUrl: string): Promise<string> => {
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 max-w-2xl mx-auto py-12 px-4">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-3 font-serif">JPG to PNG</h1>
        ...
        >
          <input ref={inputRef} type="file" accept="image/jpeg,image/png" multiple onChange={handleChange} className="hidden" />
          <div className="flex flex-col items-center">
            <Upload className="w-10 h-10 text-gray-400 mb-3" />
            <span className="text-gray-600 font-medium mb-1">Click to upload images</span>
            <span className="text-gray-400 text-sm">JPG, PNG supported</span>
          </div>
        </div>
        ...
        {files.length > 0 && !convertedUrls.length && (
          <button onClick={convertToPng} disabled={loading} className="w-full py-4 bg-blue-500 text-white font-semibold rounded-xl hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" />Converting...</> : <><CheckCircle className="w-5 h-5" />Convert to PNG</>}
          </button>
        )}
        ...
        {convertedUrls.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => convertedUrls.forEach((url, i) => downloadImage(url, i))} className="bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-600">
