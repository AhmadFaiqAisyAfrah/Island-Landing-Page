...
export default function FocusDemo() {
    ...
    const clampMinutes = (minutes: number) =>
        Math.min(MAX_DURATION_MINUTES, Math.max(MIN_DURATION_MINUTES, minutes));

    const updateMinutesFromPointer = (clientX: number, clientY: number) => {
        if (!ringSvgRef.current || isFocusing) {
            return;
        }
        ...
        let progress = Math.min(1, Math.max(0, angle / (2 * Math.PI)));
        const lastProgress = dragProgress ?? selectedMinutes / MAX_DURATION_MINUTES;

        if (lastProgress < 0.2 && progress > 0.8) {
            progress = 0;
        } else if (lastProgress > 0.8 && progress < 0.2) {
            progress = 1;
        }

        const nextMinutes = clampMinutes(
            Math.round(progress * MAX_DURATION_MINUTES),
        );
            ...
            <div className="max-w-[1200px] mx-auto px-6">
                ...
                <div className="text-center mb-16">
                    ...
                    <p className="text-xl text-[var(--paragraph-text)] max-w-2xl mx-auto leading-relaxed">
                        A simplified preview of how Island turns your focus time into a peaceful, growing world.
                    </p>
                </div>
                                    ...
                                    <>
                                        ...
                                        <circle
                                            ...
                                            onPointerDown={(event) => {
                                                ...
                                                updateMinutesFromPointer(event.clientX, event.clientY);
                                            }}
                                            onPointerMove={(event) => {
                                                if (!isDragging) {
                                                    return;
